...
export class NotesService {
  ...
  public notes: INote[] = [];
  constructor(private db: AngularFirestore, private lgs: LoginService) {
    this.notesRef = db.collection(this.lgs.user.id);
  ...
  public async createNote(newNote: INote) {
    ...
    try {
      ...
      let dRef: DocumentReference<any> = await this.notesRef.add({
        ...newNoteWithoutID,
      });
      newNote.id = dRef.id;
